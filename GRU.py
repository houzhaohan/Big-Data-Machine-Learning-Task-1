...
reframed.drop(reframed.columns[[5,6,7]], axis=1, inplace=True) #[7,8]
print(reframed.head())
...
train_X = train_X.reshape((train_X.shape[0], 1, 4)) #进行维度变换 第三个参数：特征数量
test_X = test_X.reshape((test_X.shape[0], 1, 4))
...
#模型建立
model = Sequential()
model.add(GRU(35, input_shape=(train_X.shape[1], train_X.shape[2])))
model.add(Dense(1))#定义输出层神经元个数为1个，即输出只有1维
model.add(Dropout(0.1))
sgd = optimizers.SGD(lr=0.001, decay=1e-5, momentum=0.8, nesterov=True)
model.compile(loss='mae', optimizer='adam')
# fit network
history = model.fit(train_X, train_y, epochs=250, batch_size=60, validation_data=(test_X, test_y), verbose=2,
                    shuffle=False)   #在这修改参数(hzh)
...
test_X = test_X.reshape((test_X.shape[0], 4))  # timesteps * features
# invert scaling for forecast
inv_yhat = concatenate((yhat, test_X[:, -3:]), axis=1)  #预测的 -2特征数量最后一列
inv_yhat = scaler.inverse_transform(inv_yhat)
inv_yhat = inv_yhat[:,0] #取第一列，即为氨气的浓度, mean_absolute_error
print("预测",inv_yhat)
...
pyplot.title('GRU model of NH3 value in winter')
